...
class Evaluator(object):
    ...
    def run_reconstruction_eval(self, save_submap=False):
        ...
        (self.checkpoint_path / "mesh/submaps").mkdir(exist_ok=True, parents=True)
        opt_settings = OptimizationParams(ArgumentParser(
            description="Training script parameters"))
        ...
        for idx, submap_path in enumerate(tqdm(submaps_paths)):
            submap = torch.load(submap_path, map_location=self.device)
            gaussian_model = GaussianModel()
            gaussian_model.training_setup(opt_settings)
            gaussian_model.restore_from_params(
            ...
            if save_submap:
                submap_volume = o3d.pipelines.integration.ScalableTSDFVolume(
                    voxel_length=5.0 * scale / 512.0,
                    sdf_trunc=0.04 * scale,
                    color_type=o3d.pipelines.integration.TSDFVolumeColorType.RGB8)

            for keyframe_id in submap["submap_keyframes"]:
                if keyframe_id > len(self.estimated_c2w) - 1:
                    continue
                elif keyframe_id == len(self.estimated_c2w) - 1 and not len(self.estimated_c2w) == len(self.dataset):
                    continue
                estimate_c2w = self.estimated_c2w[keyframe_id]
                estimate_w2c = np.linalg.inv(estimate_c2w)
                render_dict = render_gaussian_model(
                ...
                if save_submap: 
                    submap_volume.integrate(rgbd, intrinsic, estimate_w2c)

            if save_submap:
                submap_mesh = submap_volume.extract_triangle_mesh()
                compensate_vector = (-0.0 * scale / 512.0, 2.5 * scale / 512.0, -2.5 * scale / 512.0)
                submap_mesh = submap_mesh.translate(compensate_vector)
                submap_filename = self.checkpoint_path / "mesh/submaps" / f"submap_{idx:04d}.ply"
                o3d.io.write_triangle_mesh(str(submap_filename), submap_mesh)
                del submap_volume
        ...
        if self.config["dataset_name"] == "replica":
            evaluate_reconstruction(file_name,
                                    f"scripts/data/Replica-SLAM/cull_replica_mesh/{self.scene_name}.ply",
                                    f"scripts/data/Replica-SLAM/cull_replica_mesh/{self.scene_name}_pc_unseen.npy",
        ...
        if self.config["dataset_name"] == "scannetpp":
            ...
            for submap_path in tqdm(submaps_paths):
                ...
                for keyframe_id in submap["submap_keyframes"]:
                    ...
                    del render_dict, rendered_color, rendered_depth
                del submap
    ...
    def run(self):
        ...
        try:
            ...
            # self.run_global_map_eval(init_from='splats')  # step-5
        except Exception:
