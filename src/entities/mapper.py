...
class Mapper(object):
    ...
    def adjustment_submap(self, keyframes: list, gaussian_model: GaussianModel, iterations: int = 100) -> dict:
        """
        Optimizes the submap by refining the parameters of the 3D Gaussian based on the observations
        from keyframes observing the submap.
        Args:
        ...
        while iteration < iterations + 1:
            ...
            while self.gpu_busy.is_set(): time.sleep(0.001)
            iteration += 1
        optimization_time = time.time() - start_time
        losses_dict["optimization_time"] = optimization_time
        losses_dict["optimization_iter_time"] = optimization_time / iterations
    ...
    def map_refine(self, frame_id: int, estimate_c2w: np.ndarray, gaussian_model: GaussianModel, keyframes: list = None) -> dict:

        target_keyframes = keyframes if keyframes is not None else self.keyframes
        
        if estimate_c2w is not None:
            for i in range(len(target_keyframes)):
                kf_id = target_keyframes[i][0]
                kf_data = target_keyframes[i][1]
                
                new_w2c = np.linalg.inv(estimate_c2w[kf_id])
                kf_data["render_settings"] = get_render_settings(
                    self.dataset.width, 
                    self.dataset.height, 
        ...
        # gaussian_model.optimizer.state.clear()
        fine_iters = self.iterations * len(target_keyframes)
        
        start_time = time.time()
        opt_dict = self.adjustment_submap(target_keyframes, gaussian_model, iterations=fine_iters)
        optimization_time = time.time() - start_time
        print(f"Refine Optimization time: {optimization_time:.4f}s")

        current_kf = target_keyframes[-1][1]
        
        try:
            with torch.no_grad():
                render_pkg = render_gaussian_model(gaussian_model, current_kf["render_settings"])
                image, depth = render_pkg["color"], render_pkg["depth"]
                if current_kf["exposure_ab"] is not None:
                    image = torch.clamp(image * torch.exp(current_kf["exposure_ab"][0]) + current_kf["exposure_ab"][1], 0, 1.)
                psnr_value = calc_psnr(image, current_kf["color"]).item()
                opt_dict["psnr_render"] = psnr_value
                print(f"Refine PSNR: {psnr_value:.2f}")
                
        except Exception as e:
            print(f"[Warning] Refine visualization failed (Thread Safety Issue): {e}")
